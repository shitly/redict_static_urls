...
"""
def `view`(request):
    return HttpResponse("now\"+`view`.html)
"""
def write_url_view():
    df = url_config()
    with open("views2.txt", "w+", encoding = "UTF-8") as f:
        f.write("from django.http import HttpResponse\n\n\n")
        for i in range(len(df)):
            string = '''def %s(request):\n    return HttpResponse("appo/%s.html")\n\n\n'''%(df.at[i, "view"], df.at[i, "view"])
            f.write(string)
        f.close()
